...
class _CreatePromotionScreenState extends State<CreatePromotionScreen> {
  ...
  Future<void> _loadProducts() async {
    ...
    // Колдонуучуга таандык дүкөндү(дөрдү) табабыз
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Акция ийгилүү кошулду ✅'),
        ...
        setState(() {
          ...
          if (idx != -1) {
            _allProducts[idx]['discount_percent'] = pct;
            _allProducts[idx]['discounted_price'] = _discountedPrice;
            _allProducts[idx]['has_promotion'] = true;
  ...
  Future<void> _deletePromotion() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Акция жок кылынды 🗑️'),
        ...
        setState(() {
          final idx = _allProducts.indexWhere((p) => p['id'] == _selected!['id']);
          if (idx != -1) {
            _allProducts[idx]['discount_percent'] = null;
            _allProducts[idx]['discounted_price'] = null;
            _allProducts[idx]['has_promotion'] = false;
            _selected = _allProducts[idx];
          }
          _discountCtrl.clear();
          _discountedPrice = null;
  ...
  bool get _hasActivePromotion =>
      _selected != null && (_selected!['has_promotion'] == true);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Акция башкаруу'),
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Text(
                        '${originalPrice.toStringAsFixed(0)} сом',
                        style: AppTextStyles.bodyMedium
                ...
                if (hasPromotion)
                  Container(
                    ...
                    child: Text(
                      '-${(product['discount_percent'] as num?)?.toStringAsFixed(0)}%',
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        child: ListTile(
          ...
          title: Text(name,
              style: AppTextStyles.labelLarge, maxLines: 1,
              overflow: TextOverflow.ellipsis),
          subtitle: Text(
