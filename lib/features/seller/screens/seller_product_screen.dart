...
class _SellerProductScreenState extends State<SellerProductScreen> {
                      ...
                      children: [
                        ...
                        Column(
                          ...
                          children: [
                            ...
                            Text(
                              '${(price - discountedPrice!).toStringAsFixed(0)} ${loc.get('discount_saved')}',
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(loc.get('my_products'), style: AppTextStyles.headingSmall),
        ...
        children: [
          ...
          Container(
            ...
            child: SingleChildScrollView(
              ...
              child: Row(
                children: [
                  _categoryChip(id: null, icon: '📦', name: loc.get('prod_all'), count: _products.length),
                  ...
                  ..._allCategories.where((cat) => usedMainCatIds.contains(cat.id)).map((cat) {
                    ...
                    return Padding(padding: const EdgeInsets.only(right: 8), child: _categoryChip(id: cat.id, icon: cat.icon, name: cat.name, count: count));
                  }),
                    ...
                    ? Center(
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 16),
                            Text(
                              _selectedCategoryId == null ? loc.get('prod_empty') : loc.get('prod_empty_cat'),
                                ...
                                children: [
                                        ...
                                        SizedBox(
                                          ...
                                          child: ClipRRect(
                                            ...
                                            child: imageUrl.isNotEmpty
                                                ? Image.network(imageUrl, fit: BoxFit.cover, errorBuilder: (_, __, ___) => _noImage())
                                                : _noImage(),
                                          ),
                                        ),
                                  ...
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text(p['title'] as String? ?? '', style: AppTextStyles.labelLarge, maxLines: 2, overflow: TextOverflow.ellipsis),
                                        const SizedBox(height: 4),
  ...
  // ══ КАТЕГОРИЯ ЧИП ══
  Widget _categoryChip({required String? id, required String icon, required String name, required int count}) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(color: isSelected ? AppColors.primary : const Color(0xFFF0F0F0), borderRadius: BorderRadius.circular(20)),
        child: Row(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text(icon, style: const TextStyle(fontSize: 14)),
            const SizedBox(width: 6),
            Text(name, style: AppTextStyles.labelMedium.copyWith(color: isSelected ? Colors.white : AppColors.grey600, fontWeight: isSelected ? FontWeight.w700 : FontWeight.normal)),
            const SizedBox(width: 6),
            Container(
          ...
          child: Column(
            ...
            children: [
              ...
              ListTile(
                ...
                title: Text('🖼️  ${loc.get('prod_img_gallery')}', style: AppTextStyles.labelLarge),
                  ...
                  Flexible(
                    child: SingleChildScrollView(
                      ...
                      child: Column(
                        ...
                        children: [
                          // 1. СҮРӨТ
                          _label(loc.get('prod_field_image')),
                          GestureDetector(
                          ...
                          // 2. АТЫ
                          _label(loc.get('prod_field_name')),
                          _field(nameCtrl, loc.get('prod_hint_name')),
                          const SizedBox(height: 14),

                          // 3. БААСЫ
                          _label(loc.get('prod_field_price')),
                          _field(priceCtrl, loc.get('prod_hint_price'), type: TextInputType.number),
                          const SizedBox(height: 14),

                          // 4. НЕГИЗГИ КАТЕГОРИЯ
                          _label(loc.get('prod_field_main_cat')),
                          Container(
                            ...
                            child: DropdownButtonHideUnderline(
                              child: DropdownButton<String>(
                                ...
                                isExpanded: true,
                                items: _allCategories.map((c) => DropdownMenuItem(value: c.id, child: Text('${c.icon}  ${c.name}', style: AppTextStyles.bodyMedium))).toList(),
                                onChanged: (val) {
                          ...
                          // 5. КИЧИ КАТЕГОРИЯ
                          _label(loc.get('prod_field_sub_cat')),
                          Container(
                            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 4),
                            decoration: BoxDecoration(color: const Color(0xFFF7F7F7), borderRadius: BorderRadius.circular(12)),
                            child: DropdownButtonHideUnderline(
                              child: DropdownButton<String?>(
                                ...
                                isExpanded: true,
                                hint: Text('— ${loc.get('prod_sub_cat_general')} (${mainCat.name}) —', style: AppTextStyles.bodyMedium.copyWith(color: AppColors.grey400)),
                                items: [
                                  DropdownMenuItem<String?>(value: null, child: Text('— ${loc.get('prod_sub_cat_general')} (${mainCat.name}) —', style: AppTextStyles.bodyMedium.copyWith(color: AppColors.grey500))),
                                  ...mainCat.subcategories.map((sub) => DropdownMenuItem<String?>(value: sub.id, child: Text('${sub.icon}  ${sub.name}', style: AppTextStyles.bodyMedium))),
                                ],
                          ...
                          if (hasColors(selectedMainCatId)) ...[
                            _label(loc.get('prod_field_colors')),
                          ...
                          if (hasSizes(selectedMainCatId)) ...[
                            ...
                            _sizePicker(sizesForCategory(selectedMainCatId, selectedSubCatId), selectedSizes, setD),
                            const SizedBox(height: 14),
                          ],

                          if (hasTechFields(selectedMainCatId)) ...[
                            _label(loc.get('prod_field_brand')),
                            _field(extra1Ctrl, loc.get('prod_hint_brand_tech')),
                            const SizedBox(height: 14),
                            _label(loc.get('prod_field_model')),
                            _field(extra2Ctrl, loc.get('prod_hint_model')),
                            const SizedBox(height: 14),
                            _label(loc.get('prod_field_spec')),
                            _field(extra3Ctrl, loc.get('prod_hint_spec')),
                            const SizedBox(height: 14),
                          ],

                          if (hasBeautyFields(selectedMainCatId)) ...[
                            _label(loc.get('prod_field_brand')),
                            _field(extra1Ctrl, loc.get('prod_hint_brand_beauty')),
                            const SizedBox(height: 14),
                            _label(loc.get('prod_field_volume')),
                            _field(extra2Ctrl, loc.get('prod_hint_volume')),
                            const SizedBox(height: 14),
                          ],

                          if (hasAutoFields(selectedMainCatId)) ...[
                            _label(loc.get('prod_field_brand')),
                            _field(extra1Ctrl, loc.get('prod_hint_brand_auto')),
                            const SizedBox(height: 14),
                            _label(loc.get('prod_field_car_compat')),
                            _field(extra2Ctrl, loc.get('prod_hint_car_compat')),
                            const SizedBox(height: 14),
                          ],

                          _label(loc.get('prod_field_stock')),
                          _field(stockCtrl, loc.get('prod_hint_stock'), type: TextInputType.number),
                          const SizedBox(height: 14),

                          _label(loc.get('prod_field_desc')),
                          TextField(controller: descCtrl, maxLines: 3, style: AppTextStyles.bodyMedium, decoration: _deco(loc.get('prod_hint_desc'))),
  ...
  Widget _label(String text) => Padding(
        ...
        child: Text(text, style: AppTextStyles.labelMedium.copyWith(color: AppColors.grey600)),
      );

  Widget _field(TextEditingController ctrl, String hint, {TextInputType type = TextInputType.text}) =>
      TextField(controller: ctrl, keyboardType: type, style: AppTextStyles.bodyMedium, decoration: _deco(hint));
      ...
      children: _allColors.map((c) {
        ...
        return GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              color: isSelected ? AppColors.primary.withValues(alpha: 0.1) : const Color(0xFFF7F7F7),
              borderRadius: BorderRadius.circular(10),
            ...
            child: Row(
              ...
              children: [
                ...
                const SizedBox(width: 6),
                Text(c['name'] as String, style: AppTextStyles.labelSmall.copyWith(color: isSelected ? AppColors.primary : AppColors.grey600, fontWeight: isSelected ? FontWeight.w600 : FontWeight.normal)),
  ...
  Widget _sizePicker(List<String> sizes, List<String> selected, StateSetter setD) {
    return Wrap(
      spacing: 8, runSpacing: 8,
      children: sizes.map((s) {
        ...
        return GestureDetector(
          ...
          child: Container(
            padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8),
            decoration: BoxDecoration(color: isSelected ? AppColors.primary : const Color(0xFFF7F7F7), borderRadius: BorderRadius.circular(10), border: Border.all(color: isSelected ? AppColors.primary : Colors.transparent)),
            child: Text(s, style: AppTextStyles.labelMedium.copyWith(color: isSelected ? Colors.white : AppColors.grey600, fontWeight: isSelected ? FontWeight.w600 : FontWeight.normal)),
          ),
        );
      }).toList(),
    );
